...
func (c *rolloutContext) reconcileAnalysisRuns() error {
	...
	isRollbackWithinWindowAndValid := c.isRollbackWithinWindowAndValid()
	if isAborted || c.rollout.Status.PromoteFull || rollbackToScaleDownDelay || initialDeploy || isRollbackWithinWindowAndValid {
		c.log.Infof("Skipping analysis: isAborted: %v, promoteFull: %v, rollbackToScaleDownDelay: %v, initialDeploy: %v, isRollbackWithinWindowAndValid: %v", isAborted, c.rollout.Status.PromoteFull, rollbackToScaleDownDelay, initialDeploy, isRollbackWithinWindowAndValid)
